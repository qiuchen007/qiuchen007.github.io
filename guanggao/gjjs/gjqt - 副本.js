...
(function () {
	...
	pages[1].onCut = function () {
		...
		setTimeout( function () {
			var totalSteps = 240;
			var curStep = 0;
			var width = pages[1].querySelector( ".scroll-content" ).offsetWidth;
			...
			page2Animate = Z.requestAnimate( function () {
				if ( curStep > totalSteps ) {
					...
					pages[1].querySelector( ".scroll-content" ).zLeft = ((-width + width2 - 48) * curStep / totalSteps );
					curStep = curStep + 1;
				}
			} );
		}, 2000 );
	...
	function page8BannerList() {
		...
		if ( layoutH * 0.3968 / width > 399 / 612 ) {
			Z.insertCSSRules( {
	...
	if ( ( location.search.indexOf( "baidu" ) != -1) && (navigator.userAgent.indexOf( "MicroMessenger" ) === -1 ) ) {
		...
		pages[9].querySelector( ".yunqi-logo" ).src = "http://gujianqitan.qiniudn.com/open-yunqi-logo.png";
		// 更换图标
		var toYunQi = new Image();
		toYunQi.src = "http://gujianqitan.qiniudn.com/to-yunqi.png";
		toYunQi.classList.add( "to-yunqi" );
		toYunQi.classList.add( "hide" );
		pages[9].appendChild( toYunQi );
		...
		// 改变第10页上的图标
		pages[9].querySelector( ".page10-title" ).src = "http://gujianqitan.qiniudn.com/page10-title-inbaidu.png";
	...
	function init() {
		...
		if ( Z.ua.ios ) {
			...
			canvas.width = layoutH;
			canvas.style.width = layoutH + "px";
